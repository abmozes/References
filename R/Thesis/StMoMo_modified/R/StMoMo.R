...
#' }
#' @export
StMoMo  <- function(link = c("log", "logit", "cloglog", "logitP"), staticAgeFun = TRUE, 
  ...
  if (link == "logit") {
    ...
    gnmFormula <- "D/E ~ -1 + offset(o)"
    textFormula <- "cloglog q[x,t]"
  } else if (link == "logitP") {
    gnmFormula <- "D ~ -1 + offset(o)"
    textFormula <- "logit m[x,t]"
  }
  ...
  #Check constraint on number of simultaneous parametric and non-parametric 
  #age functions
...
print.StMoMo <- function(x, ...) {
  if (x$link == "logit" | x$link == "cloglog") {
    cat("Binomial model with predictor: ")
  } else if (x$link == "log" | x$link == "logitP") {
