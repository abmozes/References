...
#' 
#' @export
rh <- function(link = c("log", "logit", "cloglog", "logitP"), cohortAgeFun = c("1", "NP"), 
               approxConst = FALSE) {
  link <- match.arg(link)
...
fit.rh <- function(object, data = NULL, Dxt = NULL, Ext = NULL,
  ...
  if (data$type == "central" && object$link == "logit")
    warning( "Binomial model fitted to central exposure data\n")
  if (data$type == "central" && object$link == "cloglog")
    warning( "Binomial model fitted to central exposure data\n")
  if (data$type == "initial" && object$link == "log")
    warning( "Poisson model fitted to initial exposure data\n")
  if (data$type == "initial" && object$link == "logitP")
    warning( "Poisson model fitted to initial exposure data\n")
  ...
  ### Set starting values
  if (is.null(start.ax)) {
    if (object$link == "log" | object$link == "logitP") { 
      logmxt <- log(Dxt / Ext)
  ...
  Dhatxt <- Ext * mhatxt
  out$loglik <- ifelse(object$link == "log" | object$link == "logitP",
  ...
  while(absErrL > tolerance & iter < iterMax){
    ...
    sumDen <- switch(object$link, 
                     ...
                     logitP = rowSums(wxt * Dhatxt, na.rm = TRUE),
                     logit = rowSums(wxt * Dhatxt * (1 - mhatxt), na.rm = TRUE),
                     cloglog = rowSums(wxt * Dhatxt * (1 - mhatxt), na.rm = TRUE))
    ...
    sumDen <- switch(object$link, 
                     ...
                     logitP = colSums(wxt * Dhatxt * matbx2, na.rm = TRUE),
                     logit = colSums(wxt * Dhatxt * matbx2 * (1 - mhatxt), na.rm = TRUE),
                     cloglog = colSums(wxt * Dhatxt * matbx2 * (1 - mhatxt), na.rm = TRUE))
    ...
    sumDen <- switch(object$link,
                     ...
                     logitP = rowSums(wxt * Dhatxt * t(matkt2), na.rm = TRUE),
                     logit = rowSums(wxt * Dhatxt * t(matkt2) * (1 - mhatxt), na.rm = TRUE),
                     cloglog = rowSums(wxt * Dhatxt * t(matkt2) * (1 - mhatxt), na.rm = TRUE))
    ...
    if (object$cohortAgeFun == "NP"){
      ...
      sumDen <- switch(object$link,
                       ...
                       logitP = sumDiags(wxt * Dhatxt * matb0x2),
                       logit = sumDiags(wxt * Dhatxt * matb0x2 * (1 - mhatxt)),
                       cloglog = sumDiags(wxt * Dhatxt * matb0x2 * (1 - mhatxt)))
    } else {
      sumNum <- sumDiags(wxt * (Dxt - Dhatxt))
      sumDen <- switch(object$link,
                       log = sumDiags(wxt * Dhatxt),
                       logitP = sumDiags(wxt * Dhatxt),
                       logit = sumDiags(wxt * Dhatxt * (1 - mhatxt)),
                       cloglog = sumDiags(wxt * Dhatxt * (1 - mhatxt)))
    }
    ...
    if (object$cohortAgeFun == "NP"){
      ...
      sumDen <- switch(object$link,
                       ...
                       logitP = rowSums(wxt * Dhatxt * matgc2, na.rm = TRUE),
                       logit = colSums(wxt * Dhatxt * matgc2 * (1 - mhatxt), na.rm = TRUE),
                       cloglog = colSums(wxt * Dhatxt * matgc2 * (1 - mhatxt), na.rm = TRUE))
  ...
  ### Prepare output
  out$deviance <- ifelse(object$link == "log" | object$link == "logitP",
