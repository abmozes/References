...
#' Cloglog function
#' \code{cloglog} computes the cloglog function
#' @keywords internal
cloglog <- function(x) log(-log(1 - x))

#' Inverse Cloglog function
#' \code{invcloglog} computes the inverse cloglog function
#' @keywords internal
...
#' Logit function for Poisson model (link-glm)
#' \code{logitP} creates user-defined logit link for gnm/glm
#' @keywords internal
logitP <- function(Exp){
  linkfun <- function(mu) log(mu/(Exp - mu))
  linkinv <- function(eta) Exp*exp(eta)/(1 + exp(eta))
  mu.eta <- function(eta) Exp*exp(eta)/(1 + exp(eta))^2
